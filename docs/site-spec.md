...
- Home colour rhythm: hero(video) → who-we-help(coral, signature block — sits directly under
  the hero) → proof(green) → pricing(green, ivory plan cards) → services(mint) →
...
**Pricing** (added 2026-07-05 — Arya adopted a $0-down monthly model, two plans; sits before the services grid, dark green block, `id="pricing"`, component `Pricing.astro`) — eyebrow `PRICING`, H2 `Clear pricing, nothing up front.`, lede `Both plans run month to month and start at $0 down. You see the free homepage preview before you pay anything, and you can start small and move up any time.` Two **ivory cards** on the green block (v3 softened look, Arya 2026-07-05: 14px corners — cards only, buttons keep the 3px token — soft shadow, coral checkmark lists, no uppercase row labels), subgrid-aligned side by side ≥760px, stacked on mobile. Card anatomy: name (+pill) + tagline → price with struck CAD anchor → one merged terms sentence → "What's included" checklist → muted footer line. The client owns the site outright on both plans (no buyout mechanics — Arya, 2026-07-05).
1. **Growth** — $179/mo (was $249), coral border + `Start here` pill (Arya wants most clients landing here). Tagline `Your new website, live and looked after.` Terms `$0 down. We waive the $1,000 setup fee. Cancel any time.` Includes (checklist, one item per line — Arya 2026-07-05): a custom site that's yours to keep · hosting · unlimited edits through your dashboard · support when you need it. Footer `You see the free preview before you pay a dollar.`
2. **Autopilot** — $495/mo (anchor $849; Arya moved the price down from $649 and the anchor from $949 on 2026-07-05). Tagline `Everything online, handled for you.` Terms `$0 down. We waive the $2,500 setup fee. Cancel any time.` (30-days'-notice wording dropped at Arya's direction 2026-07-05.) Includes (checklist): everything in Growth · Google Business Profile · pages for each area you serve · review follow-ups · online booking · call tracking · a monthly report · Google **and Meta** ads run and tracked. (Ad-spend note removed at Arya's direction.) Footer `We guarantee more leads within 90 days. It pays for itself at about 2 jobs a month.`
...
**FounderStrip** — eyebrow `WHO'S BEHIND IT`, H2 `Meet the two engineers behind Sharplines.` Body: `Sharplines is Arya Khaligh and Amir Gholizad, two senior software engineers who spent years shipping software in tech. We now put that experience toward one problem: getting local businesses more customers. Everything we build is fast, measurable, and yours to keep.` Photos supplied (first names only in captions). Presentation v2 (Arya, 2026-07-05: the avatar strip "looked cheap"): one ivory card in the v3 card language — two rounded 4:5 portraits with sentence-case name captions, bio beside them on desktop, stacked on mobile.
...
### 6.4 About `/about`
Built 2026-07-05; copy rewritten into the site-copy voice (this section's earlier `Precision people.` H1 and bios predate the skill). H1 `The two people behind Sharplines.` (line-accent on "people"). Green hero with the two real founder portraits (4:5, card language) + free-preview CTA anchored to the on-page form + book-a-call secondary → mint story (the gap → link to live provolta.ca carrying the published 2× → direct-to-founders line, no account manager) → coral "How we work" trio (the Verifiable / Plain-spoken / Fast substance, written as `Everything is checkable` / `Plain answers` / `Software speed`) + ownership/cancel-any-time kicker linking /#pricing → name-only links to all 8 /for pages (§9 interlinking) → CTABand `variant="full"` `id="preview"`. `AboutPage` JSON-LD; founders as `Person`, first names only.
